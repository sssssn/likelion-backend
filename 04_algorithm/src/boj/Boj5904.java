...
public class Boj5904 {
    public char solution() throws IOException {
        ...
        // 최초의 수열의 길이는 3
        int length = 3;
        // 반복해서 수열을 만들어 length 가 n 보다 커지게 만들면서 length 가 moo 수열의 길이 만한 값으로 유지되기
...
//            System.out.println(length);
        ...
        // 앞쪽 또는 뒤쪽이라면, 구간을 줄여서 다시 3등분, 반복해서 가운데에 위치할 때까지 진행한다.
        // 인덱스 기준으로 찾을거므로 n의 값을 사전 조정 해준다.
        n--;
        while (true) {
            // 먼저 가운데 인덱스의 위치를 찾는다.
