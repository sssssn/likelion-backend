...
public class BinaryMaxHeap {
    ...
    // 인덱스에 존재하는 원소를 자신의 부모와 비교해서 힙의 조건을 만족시키도록 교환을 반복적으로 진행
    ...
    private void siftDown(int index) {
        while (index < size) {
            ...
            // 오른쪽도 동일, 이때 왼쪽이 더 컸다면 maxIndex 가 갱신되었을 것이고,
            // 아니라면 그대로 루트이므로 한번만 비교해도 둘 중 더 큰 것과 비교가 된다.
            if (rightChild < size && heap[rightChild] > heap[maxIndex]) {
                maxIndex = rightChild;
            }

            // 양쪽 자식과 비교했는데 최댓값의 위치가 갱신되지 않은 경우, 힙의 조건에 부합한다.
...
//        // min heap 을 사용하는 것과 비슷한 PriorityQueue
...
//        // max heap 은 Collections.reverseOrder()를 사용할 수 있다.
